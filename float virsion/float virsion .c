...
void wirteToScreen(char key);
void resetDisplay(void);
void resetTheData(int *DigitNo, char OperandsSavedData[]);
float getOP( char Op[]);
int writeCHarData( char Data[]);
float calculate( float op1,float op2, char sign);
void pulseLED(int color);
float calc(char InputEq[],int StartIndex);
...
int main(void)
{
     unsigned char key,CalcDone=0;
	  char OperandsSavedData[16]={'0','x','x','x','x','x','x','x','x','x','x','x','x','x'};
   unsigned char KeyIsPressedOnce=0,SwitchPressDetctor=0 ;
	int DigitNo=0;
		float res=0;
...
PortF_Init();
    while(1)
    {
        ...
        if(!(KeyIsPressedOnce==1)){	//toBlock multipe writing just write 1Key only per one press
	      if(DigitNo>15){resetDisplay(); writeCHarData("Error OverfLOW ");LCD_command(0xC0);  writeCHarData("PressAnyToReset"); GPIOF->DATA = 0x02; CalcDone=1;DigitNo=0;delayMs(100); }     // LED is red//show over flow error if noDigits>16 digits
				else{
											 LCD_command(0x0F);//showCursor
		...
		writeToScreen(key);
			...
			if(CalcDone==1){
			resetTheData(&DigitNo,OperandsSavedData);//reset
				sprintf(OperandsSavedData , "%f",res);
...
key=InputEq[i];
	...
	case Number :{
		if(ISop1Obtained>0)	op2[i-ISop1Obtained-1]=key;
		else op1[i]=key;
			break;
	}
		case Sign:{
...
sprintf(SavedOp1 , "%f",OP1);		//Save Op1
...
//save the Last sign 
	...
	case EqSign:{
					//calculate  call eq
					OP2=getOP(op2);
				sprintf(ResultChar , "%f",calculate(OP1,OP2,sign));
		if(SavedOp1[0]!='N'){
		OP2=calculate(OP1,OP2,sign);
				sprintf(ResultChar , "%f",calculate(getOP(SavedOp1),OP2,SavedSign));
		}
