...
PortF_Init();
    while(1)
    {
        ...
        if(!(KeyIsPressedOnce==1)){	//toBlock multipe writing just write 1Key only per one press
	      if(DigitNo>16){resetDisplay(); writeCHarData("Error OverfLOW ");LCD_command(0xC0);  writeCHarData("PressAnyToReset"); GPIOF->DATA = 0x02; CalcDone=1;DigitNo=0;delayMs(100); }     // LED is red//show over flow error if noDigits>16 digits
				else{
